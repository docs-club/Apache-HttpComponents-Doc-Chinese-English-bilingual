...
HttpClient 最基本的功能是执行 HTTP 方法。HTTP 方法的执行涉及一个或多个 HTTP 请求或 HTTP 响应的交互，这通常在 HttpClient 内部处理。用户需要提供一个要执行的请求对象，HttpClient 将请求传输到目标服务器并返回相应的响应对象，如果执行不成功，则抛出异常。
...
- **Previous Chapter：[Preface](https://github.com/clxering/Apache-HttpComponents-Doc-Chinese-English-bilingual/tree/master/HttpClient/HttpClient-Tutorial/0-Preface#preface)**
- **Next Chapter：[Chapter 2 Connection management](https://github.com/clxering/Apache-HttpComponents-Doc-Chinese-English-bilingual/tree/master/HttpClient/HttpClient-Tutorial/2-Connection-management#chapter-2-connection-management)**
