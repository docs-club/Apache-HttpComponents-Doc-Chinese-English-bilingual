...
### 7.2.1. User token handler（用户标记处理程序）
...
### 7.2.2. Persistent stateful connections（持续有状态连接）
...
## 7.3. Using the FutureRequestExecutionService（使用 FutureRequestExecutionService）
...
### 7.3.1. Creating the FutureRequestExecutionService（生成 FutureRequestExecutionService）
...
### 7.3.2. Scheduling requests（调度请求）
